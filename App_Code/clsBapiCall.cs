...
public class clsBapiCall
{
    ...
    public DataSet Get_ZBAPI_CS_GET_KIT_DETAILS()
    {
        int i = 0;
        DataSet dsOutput = new DataSet("ZBAPI_CS_GET_KIT_DETAILS");
        string strMessage = string.Empty;
        string strMessagecode = "0";
        ZBAPI_CS_GET_KIT_DETAILS objOutPut = new ZBAPI_CS_GET_KIT_DETAILS();
        DataTable[] BapiResult = new DataTable[1];
        ...
        try
        {
            ...
            if (destinationIsInialised == true)
            {
                string str = DateTime.Now.AddDays(-1).Date.ToString("yyyyMMdd");
                RfcDestination dest = RfcDestinationManager.GetDestination("P92");
                RfcRepository repo = dest.Repository;
                IRfcFunction testfn = repo.CreateFunction("ZBAPI_CS_GET_KIT_DETAILS");
                testfn.SetValue("COMPANY_CODE", "BRPL");
                testfn.SetValue("POSTING_DATE",DateTime.Now.AddDays(-1).Date.ToString("yyyyMMdd"));
                testfn.Invoke(dest);

                IRfcTable _IT_INPUT = testfn.GetTable("IT_OUT");
                dt= objOutPut.converttodotnetatble(_IT_INPUT);
            }
        }
        catch (RfcCommunicationException ex)
        {
